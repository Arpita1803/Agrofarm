  ...
  useEffect(() => {
    const loadCatalog = async () => {
      try {
        ...
        const options = Array.isArray(data?.crops) ? data.crops : [];
        setCropOptions(options);
        if (!formData.crop && options.length > 0) {
          setFormData((prev) => ({ ...prev, crop: options[0] }));
        }
      } catch (_error) {
        setCropOptions([]);
      }
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-6">
            <div>
              ...
              {cropOptions.length > 0 ? (
                <select name="crop" value={formData.crop} onChange={handleChange} required className="w-full border p-3 rounded-lg bg-white">
                ...
                <input type="text" name="crop" placeholder="e.g. Wheat" value={formData.crop} onChange={handleChange} required className="w-full border p-3 rounded-lg" />
              )}
            </div>
